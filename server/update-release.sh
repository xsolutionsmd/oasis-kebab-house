...
candidate="oasis-candidate-$$"
deployment_started=false
committed=false
had_previous=false
had_current=false
snapshot_ready=false
compose=(docker compose --project-name oasis --project-directory "$root" --env-file "$root/release.env" -f "$root/compose.yaml")
next_compose=(docker compose --project-name oasis --project-directory "$root" --env-file "$work/next.env" -f "$work/next-compose.yaml")
rollback() {
  ...
  if [[ "$had_previous" == true ]]; then
    ...
    restored_revision=$(curl -fsS --max-time 15 --resolve oasis.xsolutionsmd.com:443:127.0.0.1 https://oasis.xsolutionsmd.com/version.json | python3 -c 'import json,sys;print(json.load(sys.stdin)["revision"])') || return 1
...
cleanup() {
  ...
  if [[ "$deployment_started" == true && "$committed" != true ]]; then
    if ! rollback; then
      recovered=false
      status=1
      echo "Automatic recovery failed; recovery files retained in $work. Inspect oasis-update.service." >&2
    fi
  fi
...
if [[ "$status" == 404 ]]; then echo 'Current main is still building; retaining the existing application.'; exit 0; fi
...
if not isinstance(d['image'],str) or not re.fullmatch(r'ghcr\.io/xsolutionsmd/oasis\-kebab\-house@sha256:[0-9a-f]{64}',d['image']): raise ValueError('Invalid image')
...
revision=${release[0]}
image=${release[1]}
[[ "$revision" == "$main" ]] || { echo 'Waiting for the current main release.'; exit 0; }
...
[[ "$revision" == "$(current_main)" ]] || { echo 'Newer main exists; retaining current application.'; exit 0; }
if [[ -f "$root/compose.yaml" && -f "$root/release.env" ]]; then
  ...
  exit 1
elif [[ -n $(docker ps -aq --filter label=com.docker.compose.project=oasis) ]]; then
...
cp /usr/local/share/oasis/compose.yaml "$work/next-compose.yaml"
printf 'APP_IMAGE=%s\n' "$image" > "$work/next.env"
...
verified=false
for attempt in $(seq 1 30); do
  actual=$(curl -fsS --max-time 10 --resolve oasis.xsolutionsmd.com:443:127.0.0.1 \
    https://oasis.xsolutionsmd.com/version.json | python3 -c 'import json,sys; print(json.load(sys.stdin)["revision"])') || actual=''
  if [[ "$actual" == "$revision" ]] \
    && curl -fsS --max-time 10 --resolve oasis.xsolutionsmd.com:443:127.0.0.1 https://oasis.xsolutionsmd.com/ -o "$work/live.body" \
