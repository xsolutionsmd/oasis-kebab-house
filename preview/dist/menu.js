...
const categories = ['All dishes', 'Mains', 'Kebabs', 'Starters', 'Salads', 'Soups', 'Seafood', 'Sides', 'Desserts', 'Drinks', 'Lunch Specials'];
const imageFor = item => item.image && ['plov.webp', 'samsa.webp'].includes(item.image) ? `media/${item.image}` : null;
...
function renderCategories() {
  ...
  categories.forEach(category => {
    ...
    button.addEventListener('click', () => {
      ...
      renderCategories();
      renderMenu();
      document.querySelector('#all-dishes').scrollIntoView({ behavior: window.matchMedia('(prefers-reduced-motion: reduce)').matches ? 'auto' : 'smooth' });
    });
...
function makeDishRow(item) {
  ...
  const arrow = document.createElement('span');
  arrow.className = 'dish-row-arrow';
  arrow.setAttribute('aria-hidden', 'true');
  arrow.textContent = '↗';
  button.append(arrow);
...
function renderMenu() {
  ...
  const visible = menuItems.filter(item => (currentCategory === 'All dishes' || item.category === currentCategory) && `${item.name} ${item.description} ${item.category}`.toLowerCase().includes(query));
  document.querySelector('#menu-status').textContent = `${visible.length} ${visible.length === 1 ? 'dish' : 'dishes'} shown`;
...
function showDish(item, trigger) {
  ...
  dishDialog.querySelector('#dish-description').textContent = item.description || 'From the Oasis menu.';
  ...
  dishDialog.querySelector('.dialog-close').focus();
}

dishDialog.querySelector('.dialog-close').addEventListener('click', () => dishDialog.close());
dishDialog.querySelector('[data-close]').addEventListener('click', () => dishDialog.close());
dishDialog.addEventListener('click', event => { if (event.target === dishDialog) dishDialog.close(); });
dishDialog.addEventListener('close', () => dishTrigger?.focus());
menuSearch.addEventListener('input', renderMenu);
renderCategories();
fetch('menu.json').then(response => { if (!response.ok) throw Error('Menu unavailable'); return response.json(); }).then(items => { menuItems = items; renderMenu(); }).catch(() => { menuResults.textContent = 'Menu preview is temporarily unavailable.'; });
