...
Admin access is by Google sign-in and invitations bound to an email address. Every admin has equal permissions, including removing the initially configured admin. Invitations support Admin and Viewer and expire after 24 hours. Removed accounts lose existing sessions immediately. Viewers cannot change data. An admin cannot remove their own currently signed-in account.
